...
export default function UserPathTimeline({ path, modules = [], worksheets = [], moduleProgress = {}, worksheetScores = {} }) {
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-[#0f1d32] p-5">
      <h1 className="text-2xl font-bold text-white">{path?.title || "Learning Path"}</h1>
      <p className="mt-1 text-sm text-slate-400">{path?.description || "No active path published yet."}</p>
      ...
      <div className="mt-4 space-y-3">
        ...
        {!path?.steps?.length ? (
          <p className="rounded-xl border border-dashed border-white/20 bg-[#13243d] p-3 text-sm text-slate-400">No path steps available yet.</p>
        ) : null}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-2xl rounded-2xl border border-white/10 bg-[#0f1d32] p-5">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="text-lg font-semibold text-white">Step Details</h3>
                <p className="mt-1 text-sm text-slate-300">{viewModalData.step?.title || "Untitled Step"}</p>
              </div>
              <button
                type="button"
                onClick={() => setViewModalData(null)}
                className="rounded-lg border border-slate-500 px-3 py-1.5 text-xs text-slate-200 hover:bg-white/10"
              >
                Close
              </button>
            </div>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <section className="rounded-xl border border-white/10 bg-[#13243d] p-3">
                ...
                <div className="mt-2 space-y-1 text-sm text-slate-200">
                  {viewModalData.selectedModules.length ? viewModalData.selectedModules.map((module) => (
                    module.locked ? (
                      ...
                      <Link key={module.id} href={module.href} className="inline-flex rounded-md border border-white/20 bg-[#0f1d32] px-2 py-1 text-slate-100 hover:border-amber-300">
                        {module.title}
                      </Link>
                    )
                  )) : <p className="text-slate-400">No modules selected.</p>}
                </div>
              </section>
              ...
              <section className="rounded-xl border border-white/10 bg-[#13243d] p-3">
                ...
                <div className="mt-2 space-y-1 text-sm text-slate-200">
                  {viewModalData.selectedWorksheets.length ? viewModalData.selectedWorksheets.map((sheet) => (
                    <Link key={sheet.id} href={sheet.href} className="inline-flex rounded-md border border-white/20 bg-[#0f1d32] px-2 py-1 text-slate-100 hover:border-amber-300">
                      {sheet.title}
                    </Link>
                  )) : <p className="text-slate-400">No worksheets selected.</p>}
                </div>
              </section>

              <section className="rounded-xl border border-white/10 bg-[#13243d] p-3 md:col-span-2">
                <p className="text-xs uppercase tracking-wide text-sky-300">Info</p>
                <p className="mt-2 text-sm text-slate-200">{viewModalData.infoText || "No info content."}</p>
              </section>
            </div>
          </div>
        </div>
